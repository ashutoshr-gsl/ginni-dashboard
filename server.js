...
async function zwingFetch(apiPath, params={}) {
  const isStoreList = apiPath === '/store-list';
  const base = isStoreList
    ? 'https://lc.gozwing.com/admin'
    : 'https://lc.gozwing.com/api/v1/report';
  ...
  if (isStoreList) {
    ...
    res = await fetch(base + apiPath, { method: 'POST', headers, body: '_token='+encodeURIComponent(zwingXsrf) });
  } else {
